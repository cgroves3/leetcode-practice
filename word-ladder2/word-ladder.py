    ...
    def _getConnectedWords(self, word, wordList):
        ...
        for i in range(len(word)):
            for letter in ascii_lowercase:
                ...
                ind = bisect.bisect_left(wordList, new_word)
                if ind != len(wordList) and wordList[ind] == new_word:
                    words.append(wordList[ind])
        return words
...
def runDijkstra(graph, startingWord, endWord):
    ...
    while True:
        ...
        addNeighbors(graph, unexploredNeighbors, frontier, paths, node)
...
def addNeighbors(graph, neighbors, frontier, paths, node):
    ...
    for neighbor in neighbors:
        if not(neighbor in frontierList):
            ...
            # Append neighbor's path list if the cost is equal to current total cost
...
def mergePaths(graph, sourcePaths, targetPaths, min_cost):
    paths = []
    unmergedNodes = sourcePaths.keys()
...
if __name__ == "__main__":
    ...
    ladders = sol.findLadders(beginWord, endWord, wordList)
    expected = [["nanny","canny","candy","sandy","sands","sends","seeds","sleds","slews","clews","clefs","cleft","cleat","bleat","bloat","float","flout","clout","cloud","aloud"],["nanny","danny","dandy","sandy","sands","sends","seeds","sleds","slews","clews","clefs","cleft","cleat","bleat","bloat","float","flout","clout","cloud","aloud"]]
